...
def _handle_conv_leave(staff, user_id, state, text=None, pb_data=None):
    ...
    if step == 1:
        ...
        # 換頁：留在 step 4，更新顯示頁
        if pb_data and pb_data.startswith('lv_st_page='):
            try:
                page = int(pb_data.split('=')[1]) % 4
            except Exception:
                page = 1
            ...
            return
        # 選定開始時間
        raw_t = (pb_data[6:] if pb_data and pb_data.startswith('lv_st=')
                 else (text.strip() if text else None))
        try:
        ...
        # 換頁：留在 step 5
        if pb_data and pb_data.startswith('lv_et_page='):
            try:
                page = int(pb_data.split('=')[1]) % 4
            except Exception:
                page = 1
            ...
            return
        # 選定結束時間
        raw_t = (pb_data[6:] if pb_data and pb_data.startswith('lv_et=')
                 else (text.strip() if text else None))
        try:
            h, m = map(int, raw_t.split(':'))
            assert 0 <= h <= 23 and 0 <= m <= 59
        ...
        row = conn.execute("""
            INSERT INTO leave_requests
              (staff_id, leave_type_id, start_date, end_date, start_half, end_half,
               start_time, end_time, total_days, reason, status, created_at)
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,'pending',NOW()) RETURNING id
        """, (staff['id'], lt['id'], start_date, end_date,
              start_half, end_half, start_time, end_time,
...
def _calc_today_pay(staff):
    ...
    ot_rate1     = float(staff.get('ot_rate1') or 1.34)
...
def _calc_leave_days(start_date_str, end_date_str, start_half=False, end_half=False,
    ...
    # 時段模式：有 start_time 且 end_time
    st_min = _parse_time(start_time)
    et_min = _parse_time(end_time)
        ...
        while cur <= e:
            if _is_workday(cur):
                if cur == s and cur == e:
                    ...
                    total_min += max(0, WORK_END - st_min)
                elif cur == e:
                    total_min += max(0, et_min - WORK_START)
                else:
                    total_min += work_hours * 60
            cur += _tdd(days=1)
        return round(total_min / (work_hours * 60), 2)
...
def _auto_generate_salary(conn, staff, month, work_days=None, batch_ctx=None):
        ...
        if ot_pay == 0 and actual_work_hours > 0:
            for pd in punch_details:
                ...
                if overtime_h > 0:
                    ...
                    rate1 = float(staff.get('ot_rate1') or 1.34)
    ...
    # ── 預支薪資扣款 ────────────────────────────────────────────
    if batch_ctx is not None:
        adv_rows = batch_ctx.get('advances', {}).get(staff['id'], [])
    else:
        adv_rows = conn.execute("""
            SELECT id, amount, advance_date, note FROM salary_advances
            WHERE staff_id=%s AND deduct_month=%s AND status='pending'
        """, (staff['id'], month)).fetchall()
        adv_rows = [dict(r) for r in adv_rows]
    for adv in adv_rows:
        ...
        items.append({
            'id': f'advance_{adv["id"]}',
            'name': f'預支薪資扣款（{adv_date}）',
            'type': 'deduction',
            'amount': adv_amt,
            'formula': '',
            'calc_note': adv.get('note') or '',
            'advance_id': adv['id'],
        })
...
def api_salary_confirm_all():
    ...
    with get_db() as conn:
        rows = conn.execute("""
            ...
            RETURNING id, staff_id, month, net_pay, items
        """, (by, month)).fetchall()
        # 標記所有被確認薪資中的預支為已扣帳
        all_adv_ids = []
        for row in rows:
            try:
                items = row['items'] if isinstance(row['items'], list) else _json.loads(row['items'] or '[]')
                all_adv_ids.extend(it['advance_id'] for it in items if it.get('advance_id'))
            except Exception:
                pass
        if all_adv_ids:
            conn.execute("""
                UPDATE salary_advances SET status='deducted', updated_at=NOW()
                WHERE id = ANY(%s)
            """, (all_adv_ids,))
    confirmed = len(rows)
    for row in rows:
...
def api_salary_confirm(rid):
    ...
    with get_db() as conn:
        ...
        if row:
            ...
            try:
                items = row['items'] if isinstance(row['items'], list) else _json.loads(row['items'] or '[]')
                adv_ids = [it['advance_id'] for it in items if it.get('advance_id')]
                if adv_ids:
                    conn.execute("""
                        UPDATE salary_advances SET status='deducted', updated_at=NOW()
                        WHERE id = ANY(%s)
...
def _advance_row(row):
    if not row: return None
    d = dict(row)
    d['amount'] = float(d['amount']) if d.get('amount') is not None else 0
...
def api_salary_advances_list():
    ...
    with get_db() as conn:
        ...
        params = []
        if staff_id:
            conds.append("sa.staff_id=%s"); params.append(staff_id)
        if month:
            conds.append("sa.deduct_month=%s"); params.append(month)
        if status:
            conds.append("sa.status=%s"); params.append(status)
        where = ('WHERE ' + ' AND '.join(conds)) if conds else ''
        rows = conn.execute(f"""
            SELECT sa.*, ps.name as staff_name, ps.employee_code, ps.department
            FROM salary_advances sa
            JOIN punch_staff ps ON ps.id=sa.staff_id
            {where}
            ORDER BY sa.deduct_month DESC, sa.advance_date DESC
        """, params).fetchall()
...
def api_salary_advance_create():
    ...
    note         = b.get('note', '')
    if not staff_id or not amount or not advance_date or not deduct_month:
        return jsonify({'error': '員工、金額、預支日期、扣款月份為必填'}), 400
    with get_db() as conn:
        row = conn.execute("""
            INSERT INTO salary_advances (staff_id, amount, advance_date, deduct_month, note)
            VALUES (%s,%s,%s::date,%s,%s) RETURNING *
        """, (staff_id, float(amount), advance_date, deduct_month, note)).fetchone()
    return jsonify(_advance_row(row)), 201

@app.route('/api/salary/advances/<int:aid>', methods=['PUT'])
@require_module('salary')
    ...
    with get_db() as conn:
        row = conn.execute("""
            UPDATE salary_advances
               SET amount=%s, advance_date=%s::date, deduct_month=%s,
